...
module.exports = [
    ...
    function (pngFile, outputFile) {
        // jscs: disable
        var command = format('pngcrush -nofilecheck -bail -blacken -reduce -rem alla -force %s "%s" "%s" 2>&1',
    ...
    function (pngFile, outputFile) {
        ...
        // jscs: disable
        var  command = format('zopflipng --lossy_transparent -y --always_zopflify --splitting=%s --iterations=%s --timelimit=%s "%s" "%s"',
                splitting,
                iterations,
                timelimit,
                pngFile.name,
    ...
    /// optipng
    function (pngFile, outputFile) {
        var command = format('optipng -force -o6 "%s" -out "%s"', pngFile.name, outputFile.name);
    ...
    /// advpng
    function (pngFile, outputFile) {
        var command = format('advpng/bin/advpng -z -4 -f "%s"', outputFile.name);
